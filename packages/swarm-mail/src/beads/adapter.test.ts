 ...
 * Beads Adapter Tests
 *
 * Tests the BeadsAdapter factory and its interface implementation.
 *
 * ## Test Strategy
 * 1. Factory creation - createBeadsAdapter returns valid adapter
...
describe("Beads Adapter", () => {
  ...
  test("createBeadsAdapter - returns valid adapter", () => {
    expect(adapter).toBeDefined();
    expect(adapter.createBead).toBeFunction();
    expect(adapter.getBead).toBeFunction();
    expect(adapter.queryBeads).toBeFunction();
    expect(adapter.updateBead).toBeFunction();
    expect(adapter.closeBead).toBeFunction();
  });
  ...
  test("createBead - creates a new bead", async () => {
  ...
  test("getBead - retrieves existing bead", async () => {
    const created = await adapter.createBead(projectKey, {
      title: "Get Test",
      type: "feature",
      priority: 3,
    });

    const retrieved = await adapter.getBead(projectKey, created.id);
    expect(retrieved).not.toBeNull();
    expect(retrieved?.id).toBe(created.id);
    expect(retrieved?.title).toBe("Get Test");
  });

  test("queryBeads - returns all beads", async () => {
    await adapter.createBead(projectKey, {
      title: "Bead 1",
      type: "task",
      priority: 2,
    });
  ...
  test("updateBead - updates bead fields", async () => {
    const bead = await adapter.createBead(projectKey, {
      title: "Original",
      type: "task",
      priority: 2,
    ...
    const updated = await adapter.updateBead(projectKey, bead.id, {
      title: "Updated",
  ...
  test("changeBeadStatus - changes bead status", async () => {
    const bead = await adapter.createBead(projectKey, {
      title: "Status Test",
      type: "task",
      priority: 2,
    ...
    const updated = await adapter.changeBeadStatus(projectKey, bead.id, "in_progress");
    expect(updated.status).toBe("in_progress");
  });

  test("closeBead - closes a bead", async () => {
    const bead = await adapter.createBead(projectKey, {
      title: "Close Test",
      type: "task",
      priority: 2,
    ...
    const closed = await adapter.closeBead(projectKey, bead.id, "Completed");
    expect(closed.status).toBe("closed");
    expect(closed.closed_reason).toBe("Completed");
    expect(closed.closed_at).toBeGreaterThan(0);
  });

  test("reopenBead - reopens a closed bead", async () => {
    const bead = await adapter.createBead(projectKey, {
      title: "Reopen Test",
      type: "task",
      priority: 2,
    ...
    await adapter.closeBead(projectKey, bead.id, "Done");
    const reopened = await adapter.reopenBead(projectKey, bead.id);
  ...
  test("addDependency - adds a dependency", async () => {
    const bead1 = await adapter.createBead(projectKey, {
      title: "Blocker",
      type: "task",
      priority: 2,
    });
    const bead2 = await adapter.createBead(projectKey, {
      title: "Blocked",
      type: "task",
      priority: 2,
    });
  ...
  test("getEpicChildren - returns epic children", async () => {
    ...
    const child = await adapter.createBead(projectKey, {
      title: "Subtask",
      type: "task",
      priority: 2,
      parent_id: epic.id,
  ...
  test("isEpicClosureEligible - returns true when all children closed", async () => {
    ...
    // Close child
    await adapter.closeBead(projectKey, child.id, "Done");
  ...
  test("getNextReadyBead - returns unblocked bead", async () => {
    await adapter.createBead(projectKey, {
      title: "Ready Bead",
      type: "task",
      priority: 1,
    ...
    const ready = await adapter.getNextReadyBead(projectKey);
    expect(ready).not.toBeNull();
    expect(ready?.status).toBe("open");
  });

  test("getInProgressBeads - returns in-progress beads", async () => {
    const bead = await adapter.createBead(projectKey, {
      title: "WIP Bead",
      type: "task",
      priority: 2,
    ...
    await adapter.changeBeadStatus(projectKey, bead.id, "in_progress");

    const inProgress = await adapter.getInProgressBeads(projectKey);
